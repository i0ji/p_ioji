...
export default function Card({ data }: { data: heroModel }) {
  ...
  return (
    <div
      className="mx-auto my-4 flex h-[70vh] w-[90vw] transform flex-col justify-between rounded-lg bg-gray-800 p-4 text-stone-400 shadow-lg drop-shadow-lg transition-transform duration-200 ease-linear hover:scale-[1.01] 2xl:w-[30vw]"
      data-carousel-item
    >
      <h6 className="mb-2 flex h-auto text-xl font-bold">{data.title}</h6>
      <div className="xl hidden h-auto flex-auto flex-col justify-between 2xl:lg:block">
      ...
      <div className="flex h-auto flex-col justify-between">
        ...
        <button
          ...
          className="group relative me-2 mt-2 inline-flex w-full items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br from-purple-600 to-blue-500 p-0.5 text-sm font-medium text-gray-900 hover:text-white focus:outline-none focus:ring-4 focus:ring-blue-300 group-hover:from-purple-600 group-hover:to-blue-500 2xl:hidden dark:text-white dark:focus:ring-blue-800"
        >
